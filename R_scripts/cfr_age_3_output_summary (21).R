...
# Load functions
source("source/mcmc_summary21.R")
source("source/mcmc_and_likelihood_functions21.R")

# Read input raw data
data <- readRDS("data/age_disaggregated_onset_incidence_data21.rds")
observed_deaths <- c(10, 11 , 51 , 183, 810 , 3139, 9163, 23524, 60086)
observed_cases <- c(134274, 261679, 349978, 364515, 470588, 520582, 329011, 244802, 277591)
prop_lombardy <- 28738 / 96977
crude_cfr <- 96977 / 2953120

# Read raw MCMC output and inputs used to run the MCMC
m <- readRDS("data/MCMC_fitting_output21.rds")
# Data for prediction from likelihood function
data_list <- readRDS("data/predicted_x21.rds")
...
best_y <- min(data[data$location == "Lombardy", "nici"])
...
predicted <- bind_rows(lapply(1:nrow(m_thin), function(x, data2, m_thin, best_z){
  ...
  data2 %>% mutate(
    age60 = ifelse(age_groups %in% c("0-9", "10-19", "20-29", "30-39",
                                     "40-49", "50-59"), "U60", "O60"),
    cases_adjusted = cases * ifelse(location == "Lombardy", 1 /best_y, 1 /best_z) * nici,
...
### Hospitalisation ############################################################
# From ISS
proportion_observed_cases_severe <- c(0.0052, 0.0033, 0.007, 0.0127, 0.0170,0.03, 0.0574, 0.1091, 0.1364)
