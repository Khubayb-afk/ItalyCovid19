...
population <- read.csv("data/italy_population_demography21.csv") 
Epicentro_report_onset <- read.csv("data/Epicentro_onset_data21.csv")

# Setting Static Variables
smoothing_centre <- as.Date("2021-03-01", format = "%Y-%m-%d")
...
##    Feb1st. Initial linear interpolation of case numbers between                ##
##    Feb 28th and 1st Mar, followed by distribution of cases                   ##
...
# Smoothing of Incidence Spike Around 1st March 
...
## Linear interpolation of cases on 1st Mar from cases on 28th feb & 2nd Mar
cases_31st_Jan <- cases_by_onset[cases_by_onset$Date == "2021-02-28", 2:4]
cases_1st_Mar <- cases_by_onset[cases_by_onset$Date == "2021-03-01", 2:4]
cases_2nd_Mar <- cases_by_onset[cases_by_onset$Date == "2021-03-02", 2:4]
adj_cases_1st_Mar <- (cases_31st_Jan + cases_2nd_Mar)/2 # linear intepolation

## Redstributing extra cases to surrounding 3 days either side & 1st Feb equally 
cases_by_onset[cases_by_onset$Date == "2021-02-28", 2:4] <- adj_cases_1st_Mar
cases_to_distribute <- (cases_1st_Mar - adj_cases_1st_Mar)/total_days_smoothing
...
Lombardy_case_age<- c(4.3, 8.5, 11.9, 12.3, 15.9, 17.8, 11.2, 8.4, 9.8) 
Lombardy_case_age<- Lombardy_case_age/100 # convert percentage to proportion
Italy_case_age <- c(4.3, 8.5, 11.9, 12.3, 15.9, 17.8, 11.2, 8.4, 9.8) 
...
saveRDS(age_disaggregated_counts_df, file = "data/age_disaggregated_onset_incidence_data21.rds")
